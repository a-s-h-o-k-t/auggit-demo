...
export class SoupdateComponent implements OnInit {
  ...
  constructor(public salesapi: SalesService, public soapi: SoService, public fb: FormBuilder, 
    public dialog: MatDialog,public api:ApiService,public activatedRoute:ActivatedRoute, public router: Router) {} 
  ...
  // {
  //   // return new Promise((resolve) => { 
  //   //   this.soapi.getMaxInvoiceNo(type).subscribe(res=>{
  //   //       this.pono = res;
  //   //       resolve({ action: 'success' });
  //   //   })
  //   // });
  ...
  {
    return new Promise((resolve) => { 
      this.soapi.get_SO(this.pono).subscribe((res)=>{  
        console.log("po",res);        
  ...
  loadSODetails()
  {
    return new Promise((resolve) => { 
      this.emprtyGrid();
      this.soapi.get_SODetails(this.pono).subscribe(res=>{          
        console.log(res);  
        res.forEach((v:any) => {                      
  ...
  {
    return new Promise((resolve) => { 
      this.soapi.Delete_SO(this.pono).subscribe(res=>{
         this.soapi.Delete_SODetails(this.pono).subscribe(res=>{
            this.soapi.Delete_SOCusFields(this.pono).subscribe(res=>{           
              resolve({ action: 'success' });
            }); 
         });                  
      })         
