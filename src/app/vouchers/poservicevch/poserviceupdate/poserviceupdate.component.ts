...
export class PoserviceupdateComponent implements OnInit {
  ...
  constructor(public poapi: PoserviceService, public fb: FormBuilder, public dialog: MatDialog,
  ...
  {
    // return new Promise((resolve) => { 
    //   this.poapi.getMaxInvoiceNo(this.vchType).subscribe(res=>{
    //       this.pono = res;
    //       resolve({ action: 'success' });
    //   })
    // });
  ...
  loadGRN()
  {
    ...
    this.poapi.get_PO(this.pono).subscribe((res)=>{  
      ...
      this.roundedoff = res[0].roundedoff;this.pono
  ...
  loadSavedSdef()
  {
    this.poapi.getSavedDefFields(this.pono).subscribe((res) => {
      ...
      if(this.sdef.length==0){ this.loadsdef();}      
    });
  }
  ...
  {
    return new Promise((resolve) => { 
      this.poapi.Delete_PO(this.pono, this.vchType).subscribe(res=>{
         this.poapi.Delete_PODetails(this.pono, this.vchType).subscribe(res=>{             
              this.poapi.deleteDefFields(this.pono ,this.vchType).subscribe(res=>{            
                 resolve({ action: 'success' });
              });    
           });    
         });                  
  ...
  {    
    ...
    {  
      ...
      {
        this.loading = true; 
        this.deleteExistingPO().then(res=>{             
  ...
  showSuccessMsg()
  {
    let dialogRef = this.dialog.open(SuccessmsgComponent,
      ...
      dialogRef.afterClosed().subscribe(result=>{
        ...
        this.router.navigateByUrl("/poservicelist");
      });
  }
